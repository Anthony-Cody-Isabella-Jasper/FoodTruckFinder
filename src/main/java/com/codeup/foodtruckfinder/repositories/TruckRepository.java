...
public interface TruckRepository extends JpaRepository<Truck, Long> {
    ...
    @Query(value = "SELECT DISTINCT t.id, t.description, t.latitude, t.longitude, t.name, t.phone, t.profile_picture, t.truck_owner_id FROM trucks t JOIN menu_items mi on t.id = mi.truck_id WHERE t.name LIKE CONCAT('%', ?1, '%') OR mi.description LIKE CONCAT('%', ?1, '%')", nativeQuery = true)
